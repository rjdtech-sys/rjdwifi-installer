...
function getRelayInactiveValue() {
  return relayActiveHigh ? 0 : 1;
}
...
async function initGPIO(
  ...
  if (relayOutput) {
    try {
      relayOutput.writeSync(getRelayInactiveValue());
    ...
    if (boardType === 'orange_pi') {
      if (boardModel) {
        ...
        if (relayMapped !== undefined && relayMapped !== null) {
          relaySysPin = correctOpiGpioNumber(relayMapped);
          relayPhysPin = relayPin;
          console.log(
            `[GPIO] OPi ${boardModel}: Relay Physical Pin ${relayPin} -> raw=${relayMapped} -> sysfs=${relaySysPin}`
          );
        } else {
          console.warn(
  ...
  if (Gpio && relaySysPin !== -1) {
    try {
      ...
      const initialValue = getRelayInactiveValue();
...
function setRelayState(isOn) {
  ...
  // Literal relay trigger mode:
  // - Active High: trigger/open coinslot with GPIO HIGH, idle with GPIO LOW.
  // - Active Low: trigger/open coinslot with GPIO LOW, idle with GPIO HIGH.
